...
| Desktop build/package | v0.3.0 local candidate verified | `npm run package:windows` built and audited an unsigned x64 NSIS installer, zh-CN MSI, and portable EXE; package smoke and installed-app acceptance verified the manifest-selected NSIS | MSI installation, code signing, and public distribution remain separate |
...
| Windows distribution | v0.3.0 unsigned local evidence | The current candidate produced NSIS, zh-CN MSI, portable EXE, hashes, audit, automated package lifecycle evidence, and installed-app acceptance | No-delivery-after-quit observation, 200% scaling, Narrator, Authenticode certificate, and SmartScreen reputation |
