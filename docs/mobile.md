...
报告和截图保存到显式输出目录；选择器、进程重启、身份校验或页面错误都会使验收失败。此脚本的单元测试只证明失败关闭约束；在对应提交的隔离模拟器工作流实际成功前，产品闭环保持 `NOT_RUN`。它不证明复习到期后的执行、整机重启、物理设备或商店交付。仅对专用隔离模拟器运行，不使用个人设备或已有用户 AVD。
