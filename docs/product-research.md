...
当前实现已完成“轻量标签与全局搜索”和“学习习惯作为派生视图”的首个闭环：标签写入遵守 CAS、幂等、审计与可撤销边界，完成记录保留标签快照；搜索从七个既有入口旁的统一命令打开，并能准确返回任务详情或指定完成记录。“节律”位于学习二级导航，只统计真实重复实例与一对一完成证据，单独提示缺证据完成；从任务表面完成学习实例时必须先填写收获、证据和下一步。“回顾”的本周证据按主题归集当前计划现状、当前周完成记录、这些记录关联且去重的专注分钟，以及当前周完成的复习；计划结果区分完成、取消和跳过，点击指标可返回原任务、精确发生项或完成记录。计划时间、预计时长和主题缺少事件快照，因此这里明确展示当前 Workspace 对本周的视图，不能冒充历史原计划；无结束时间的 Session 也不被宣称为完整实际投入。保存筛选器、证据覆盖、到期复习和重复卡点等更完整统计仍按后续条目推进。
