...
当前实现已完成“轻量标签与全局搜索”和“学习习惯作为派生视图”的首个闭环：标签写入遵守 CAS、幂等、审计与可撤销边界，完成记录保留标签快照；搜索从七个既有入口旁的统一命令打开，并能准确返回任务详情或指定完成记录。“节律”位于学习二级导航，只统计真实重复实例与一对一完成证据，单独提示缺证据完成；从任务表面完成学习实例时必须先填写收获、证据和下一步。“回顾”的本周证据按主题归集当前周完成记录、这些记录关联且去重的专注分钟，以及当前周完成的复习；点击指标只显示构成它的原始完成记录，并能继续打开原任务。它不把无结束时间的 Session 宣称为完整实际投入。保存筛选器，以及计划/取消、证据覆盖和重复卡点等更完整统计仍按后续条目推进。
