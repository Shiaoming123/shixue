...
早先 PR4 本地工作树曾生成 Portable、NSIS、MSI 三种未签名制品，并留下自动 package smoke 的历史证据；这些结果不能作为 v0.3.0 候选的验收结果。v0.3.0 候选已生成并审计 `unsigned-local` 制品；精确 NSIS 的自动 smoke 与已安装应用基础验收覆盖可见窗口、单实例、深色重启、真实 Win32 快捷键、日历模式和卸载清理，结果为 `PASS`，并于 2026-09-07 发布无签名 Portable、NSIS 和 MSI。最终重建 SHA 的复习完成 UI、提醒/托盘动作、MSI 安装、退出后不再投递、200% 缩放和 Narrator 仍按 [v0.3.0 验收账本](./releases/v0.3.0-acceptance.md)保留为 `NOT_RUN`。
