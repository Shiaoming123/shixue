...
| 主题控件基础 | 已实现 | 已有 overlay、popover、listbox、checkbox、switch、dialog、sheet、toast 与日期时间控件；共享宿主管理 Escape、外点策略、焦点闭环与恢复，未运行的原生交互/视觉检查不写成已验证 |
...
`WorkspaceStateV3` 为后续能力预留集合不等于对应业务已经交付。重复规则与发生项、离线自然语言快速新增已实现并通过 focused tests 与真实 Web smoke；共享投递账本、多提醒规则、应用内动作卡、按需通知权限、托盘关闭策略与显式开机启动设置也已实现。当前日历分支增加日/周/月/议程、未安排任务托盘、预览式指针放置/调整和完整键盘替代，并通过隔离 Web 持久化 smoke。v0.3.0 精确最终候选已另行通过日历原生壳基础验收；该 SHA 的复习完成 UI、应用内提醒动作、真实托盘交互、Windows 系统通知投递、退出后不再投递和 200% 系统缩放仍为 `NOT_RUN`。Agent 行为仍是计划项；`source: agent` 只是能力信封的调用来源标记，不代表已经存在自动规划或执行 Agent。
...
TickTick 的公开产品形态以其[功能页](https://ticktick.com/features)、[Windows 页面](https://ticktick.com/windows)、[更新记录](https://ticktick.com/public/changelog/en.html)与[帮助中心](https://help.ticktick.com/)为准。当前实现已借鉴三栏桌面信息架构、移动端单栏推进、智能清单、清单管理、日期与多级优先级，并交付日历多视图、重复规则、离线自然语言快速新增和多提醒。协作、习惯、倒数日与 AI 执行仍不在本阶段。未登录付费账号，因此付费边界与登录后拖动手感不作为已验证事实。
...
| 编辑 | 标题/笔记失焦保存，其他字段立即保存 | 重写为显式草稿和有序提交，避免静默丢失部分失败 | 标题、笔记、清单、计划/截止日期、精确提醒、优先级、时长、完成标准先在本地草稿校验；提交失败保留未完成步骤，重试不重复已成功写入 |
...
| 全局捕捉 | 独立 always-on-top 窗口 | 使用现有主窗口与模块契约重写，减少窗口状态复杂度 | 桌面 `Ctrl+Alt+A` 已通过 source/mock 与真实 Win32 输入的显示/恢复/聚焦验收 |
| 提醒 | Rust 后台每 20 秒检查，窗口隐藏时仍可提醒 | 用共享能力账本解决规则、认领、确认与恢复；Rust 只发 tick | 多条开始/截止偏移或绝对提醒可独立排程与稍后提醒；最终重建 SHA 的应用内到期卡、完成、稍后、Windows 系统通知投递与退出后不再投递为 `NOT_RUN`；原生动作按钮为 `UNSUPPORTED` |
...
| 移动端 | 小屏隐藏部分导航且无完全替代 | 做得更好 | 紧凑底栏保留收件箱、今天、日历、清单、学习五个入口，最近 7 天与已完成经“清单/更多”可达；详情使用 Sheet，筛选/批量操作可触控且不横向遮挡 |
| 无障碍 | 有焦点环；dialog、嵌套交互、拖拽仍有缺口 | 做得更好 | 语义化 button/input、主题化 Listbox/Checkbox/Switch、显式 label、dialog/alertdialog、aria-live、44px 触控目标、reduced-motion；未经适配的可见原生控件属于验收失败 |
...
| 9 | 快捷键 | 输入控件内不劫持；`/`/`N`/`J`/`K`/`E`/`?` 可用；全局快捷键只在桌面注册一次 | 页面快捷键与真实 Win32 `Ctrl+Alt+A` 的原生 focus/restore 已通过 |
| 10 | 提醒 | 多规则独立投递；稍后提醒不改任务日程；共享认领/确认可重试；启动不请求权限；应用内动作幂等 | 领域、模块与界面合同已通过；最终重建 SHA 的安装态应用内到期/稍后/完成、Windows 系统通知和退出后不再投递为 `NOT_RUN`；原生动作按钮 `UNSUPPORTED` |
| 11 | 账号同步 | 默认关闭；无配置/会话时零请求；冲突可解释；token 仅在 Rust/keyring；RLS 隔离用户 | 代码与离线门禁已通过；真实 Supabase/RLS/双设备 E2E 为 NOT_RUN |
| 12 | 响应式与无障碍 | 日历五个 Web 固定视口实测；主题化日期 `grid`、桌面 Popover/移动 Sheet 无裁切和页面横向滚动；无 console/page error | Web 已通过；Windows 安装态实测为 125%，200% 缩放/高对比仍为 `NOT_RUN` |
...
2026-09-06 本分支 Task2 的发布 bundle 性能基准在 Windows 10.0.22631、Node 24.14.1、x64、AMD Ryzen 7 9850X3D、`logicalCpuCount=16` 上运行，fixture 包含 10,000 个任务、1,000 个活动重复系列和 50,000 个发生项。一次不计时预热后运行 7 次样本并取中位数；Today 为 7.9 ms、7 天为 7.6 ms、日历 42 天范围为 135.9 ms，对应确定投影数量分别为 3,000、6,000、41,960，均低于 100/100/150 ms 门槛。该结果只记录数量与环境，不输出任务内容，也不代表其他硬件具有相同耗时。

## 6. 下一阶段详细方案

2026-09-04 的产品访谈已确认“时间规划优先”的六 PR 路线。v2→v3 数据迁移、能力协议与现有写入切换、统一控件基础、重复发生项、离线自然语言快速新增、多提醒与托盘业务已在对应阶段落地；日历工作区实现日/周/月/议程和未安排任务交互，并完成上述 Web 验收。v0.3.0 精确最终候选已通过 Windows 安装应用基础、日历模式和真实快捷键验收，并于 2026-09-07 作为无签名 Release 发布；该 SHA 的复习完成 UI、提醒/托盘动作、签名、更新器、200% 缩放、Narrator、Windows 系统通知和退出后不再投递仍按证据边界保持 `NOT_RUN` 或 `BLOCKED`。发布后的 Android 模拟器证据不追溯升级 v0.3.0，也不证明真机或发布。Agent 行为仍是计划，不因 v3 类型或命令来源占位而视为已交付。总规格见[拾学通用待办与时间规划基础规格](./superpowers/specs/2026-09-04-shixue-time-planning-foundation.md)。根目录 `DESIGN.md` 的 `LOCKED / NAVIGATION AMENDED` 合同与 `VISUAL_QA.md` 的已确认代表稿共同约束后续实现；新增模式或重大偏离仍需重新确认。
