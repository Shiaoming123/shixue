...
2026-09-04 的产品访谈已确认“时间规划优先”的六 PR 路线。v2→v3 数据迁移、能力协议与现有写入切换、统一控件基础、重复发生项、离线自然语言快速新增、多提醒与托盘业务已在对应阶段落地；日历工作区实现日/周/月/议程和未安排任务交互，并完成上述 Web 验收。v0.3.0 精确最终候选已通过 Windows 安装应用基础、日历模式和真实快捷键验收；它尚未发布，该 SHA 的复习完成 UI、提醒/托盘动作、签名、更新器、200% 缩放、Narrator、Windows 系统通知、退出后不再投递和 iOS/Android 原生流程仍按证据边界保持 `NOT_RUN` 或 `BLOCKED`。Agent 行为仍是计划，不因 v3 类型或命令来源占位而视为已交付。总规格见[拾学通用待办与时间规划基础规格](./superpowers/specs/2026-09-04-shixue-time-planning-foundation.md)。根目录 `DESIGN.md` 的 `LOCKED / NAVIGATION AMENDED` 合同与 `VISUAL_QA.md` 的已确认代表稿共同约束后续实现；新增模式或重大偏离仍需重新确认。
