...
发布 bundle 性能基准在 Windows 10.0.22631、Node 24.14.1、x64、AMD Ryzen 7 9850X3D、`logicalCpuCount=16` 上运行，fixture 包含 10,000 个任务、1,000 个活动重复系列和 50,000 个发生项。每轮一次不计时预热后运行 7 次样本并取中位数；三次连续 post-fix 验证的 median 范围为 Today 7.9–8.9 ms、7 天 7.1–7.3 ms、日历 42 天范围 135.0–144.0 ms，对应确定投影数量分别为 3,000、6,000、41,960，均低于 100/100/150 ms 门槛。该结果只记录数量与环境，不输出任务内容，不代表其他硬件具有相同耗时。
