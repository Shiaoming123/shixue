...
- 功能层统一使用两级材质：侧栏、工具栏和移动导航使用 `thin`；Sheet、Dialog、Popover、日历上下文和短暂操作条使用 `regular`。透明表面、低亮度边缘、顶部内高光和宽柔阴影共同表达层级。
- 任务行、日历网格、设置正文、表单字段和阅读内容属于内容层，保持不透明或近乎不透明；禁止逐行玻璃、逐卡强阴影和承载布局的缩放动画。
- 页面导航不再对整个 `main` 做淡入。Sheet 在离场时固定关闭前 placement；内联详情同步移除，覆盖式浮层只做局部 180–240ms 的透明度与小位移。
- 浅色、深色和自定义主题继续只写现有语义色，材质由共享 token 派生。减少透明度、手动关闭玻璃和强制色模式必须回退为不透明表面；减少动效必须关闭位移动画。
...
- 依据：[soft-surface-ui](https://github.com/Shiaoming123/agent-skills/tree/main/skills/soft-surface-ui)，风格迁移模式。
- 白底/表面 `#ffffff`，次表面 `#f6f7f9`，正文 `#27272a`，辅助 `#62626b`，边线 `#ecebef`，主蓝 `#176b91`。辅助文字与蓝色加深以满足小字号对比度。
- 深色采用 `#141619 / #1c1f23 / #272b30` 三层中性表面、`#edf0f3` 正文、`#a8afb8` 辅助与 `#84c9e6` 主蓝。状态色仍表达真实结果。
- 主容器 24px 圆角，控件 8–12px；内间距 20–24px。任务行保持平面分隔，单行至少48px。系统无衬线字体；不下载图片或字体。
- 轻边界和微阴影仍用于内容层；导航、工具条、详情浮层按上方 Glass Chrome 合同使用共享材质。深色不直接反相。
- 按压180ms、页面/列表240ms、表面360ms，`cubic-bezier(.22,1,.36,1)`；相较研究示例700ms缩短以适合高频操作。只过渡透明度/小位移，不缩放文字；reduced-motion关闭运动。
- 验收覆盖收件箱、今天、最近七天、清单、已完成、日历、主题、节奏、复习/记录、专注、设置、详情及编辑浮层，桌面和窄屏深浅色。Web预览使用独立端口隔离浏览器存储；不宣称验证原生通知或手机手势。

下文保留原正式版本合同作为对照。

- 模式：`EXISTING`（基于现有拾学视觉演进）
- 状态：`LOCKED / NAVIGATION AMENDED`（桌面方向于 2026-09-04 确认；跨端设计系统与可定制侧栏/独立设置页于 2026-09-05 按用户要求纳入）
...
2026-09-05 PR4 对比度修订：只加深浅色的既有辅助/功能色，使小字号文字在 bg/surface/surface-alt 上达到 4.5:1；危险按钮使用 `--danger-text`，避免深色浅红底上继续使用白字。高对比边界 token 必须覆盖运行时 inline 色值，forced-colors 使用系统色。开关未选中时保留可辨认轮廓。此次为可访问性修复，品牌、材质用途与布局不变。

数字日期、时长继续使用 tabular-nums；正文保留比例字体。实际代码/标识符才按逐字符辨识需求使用等宽字体，不整页切换等宽。
...
- 左栏展开态 224–248px、图标态 72px；用户可在两态间切换，选择按设备持久化，主区最小 480px；详情 320–380px，可关闭。
- 同一侧栏分组内的菜单项允许拖动排序，顺序是本机界面偏好，不改变清单领域数据；拖动必须提供 `Alt+↑/↓` 键盘替代，设置入口固定在底部。
- 列表行默认 48px；含两行元信息时 64px。点击行打开详情，复选框只切换完成。
- Today 按“逾期 / 今天计划 / 今天截止 / 今天重复”分组；同一投影项只出现一次，显示多个来源标记。

### 中等窗口 820–1279px

- 左栏保持用户选择的展开态或 72px 图标态，折叠控件始终可达；详情改为右侧覆盖抽屉，不压缩主区低于 480px。
...
- 每个页面顶部只保留一个页面标题；智能清单切换使用标题右侧的图标菜单，不再用第二个标题式选择器。
- 设置是与收件箱、今天等同层级的独立页面，不使用右侧抽屉；外观、透明度、侧栏、快速新增、提醒、数据和可用的云端能力按分区呈现。
- 日期、重复和提醒选择器复用同一弹层视觉与键盘语法。
- 日历拖动先显示半透明预览，释放后走能力命令；失败则回弹并显示原因。
- 单项可逆写入立即反馈并提供撤销；批量、删除、导入覆盖和系列范围修改先展示影响摘要。
- 导入先只读完整校验，确认面板显示备份日期及当前/候选的清单、任务、完成记录数量；取消不写入，失败保留候选以便重试。恢复演示内容显示将替换的实际数量。
- 覆盖详情、编辑及其选择浮层共用焦点栈；只有最上层处理 Escape/Tab，后台快捷键在活动浮层期间停用。Listbox 的 Tab/Shift+Tab 回到触发器所在表单的相邻控件。非法日期/时间保留输入并报错，不能自动改成另一时刻。
- 多提醒每条规则独立保存/停用，贪睡只改投递时间，不改变任务日期；学习提醒的完成必须进入证据式完成流程。权限只在首次设置或显式测试时请求。原生动作不支持时如实显示应用内动作，不宣称原生点击回调已实现。
