...
| Android device and delivery | An automated emulator-reboot persistence gate now exists, but this commit has no native run result for it, so it remains `NOT_RUN`. Physical devices, native notifications, signing, Google Play, and store delivery are also `NOT_RUN`. |
