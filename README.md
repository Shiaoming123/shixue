...
- **离线快速新增**：确定性的中英文规则在本机解析日期、时间、截止、优先级、重复、`#标签` 与 `@清单`；默认创建普通任务，也可通过明确的“学习任务”按钮创建学习任务，不根据标题猜测模式。模糊或冲突结果留给用户确认，不依赖模型或网络。
- **轻量标签与全局搜索**：标签支持创建、重命名、归档与任务关联，归档不会抹掉历史关联；`Ctrl/Cmd+K` 可从现有页面搜索任务字段、检查项、标签及完成记录中的收获、证据、卡点和下一步，并组合主题、状态、日期和标签筛选。
- **多提醒与托盘**：任务可设置多个提醒；应用运行或驻留托盘且系统权限可用时才尝试提交系统通知，完全退出后不保证提醒。系统原生动作不可用时由应用内卡片提供完成、稍后和打开任务。
- **可选学习模式**：为任务增加主题、专注会话、完成标准、成果证据和 1 / 3 / 7 天复习；从列表、详情或键盘完成学习任务时统一先填写完成证据，批量完成不会跳过这一步。“节律”按真实重复实例与完成证据派生本周进度，“回顾”按主题汇总本周计划现状、完成证据覆盖、本周到期复习、有证据完成、关联专注分钟和实际完成复习，并可下钻到原任务、发生项或完成记录。两者都不建立新的统计或空打卡事实。普通待办不要求填写学习字段。
- **本地数据演进**：WorkspaceState v3 支持 Study v1/v2 迁移与导入；IndexedDB / SQLite 在迁移前保留旧快照。JSON v3 是可重新导入的完整备份，未删除的完成记录还可单独导出为确定性的 Markdown 阅读版。旧版本应用不能读取 v3 数据，升级前备份不包含升级后的新增记录，也不提供自动降级恢复。
...
- JSON 导出用于完整备份或在 Web/桌面运行时之间手动迁移；Markdown 导出只提供无需拾学即可阅读的完成记录，不可用于恢复。两端不会自动互相同步。
...
| Android 真机与发布 | `20000be` 的 `android-debug` run [34182148062](https://github.com/Shiaoming123/shixue/actions/runs/34182148062) 已在 API 35 x86_64 模拟器通过整机重启后的 SQLite 任务、创建回执和事件恢复，并确认内核 boot ID 已变化；真机、原生通知、签名、Google Play 和商店交付仍为 `NOT_RUN`。 |
...
- **领域层**：`WorkspaceStateV3` 是工作区唯一事实源；UI 只能调用应用能力与领域命令，不能绕过状态机直接修改持久化状态。
...
- [x] 可逆标签管理与跨任务、检查项、标签、完成记录的确定性全局搜索
