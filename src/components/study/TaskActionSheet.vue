...
input, textarea { width: 100%; min-height: var(--field-min-height); padding: 9px 12px; border: 1px solid transparent; border-radius: var(--radius-md); outline: 0; background: var(--field-fill); color: var(--text); font-size: 13px; transition: border-color var(--motion-fast) var(--ease), box-shadow var(--motion-fast) var(--ease), background var(--motion-fast) var(--ease); } textarea { min-height: 76px; resize: vertical; line-height: 1.5; } input:hover:not(:focus), textarea:hover:not(:focus) { background: var(--field-hover-fill); } input:focus, textarea:focus { border-color: var(--accent); background: var(--field-focus-fill); box-shadow: var(--field-focus-ring); }
...
@media (max-width: 819px) { .date-grid { grid-template-columns: 1fr; } footer { align-items: stretch; flex-direction: column-reverse; }.save { width: 100%; min-height: 52px; }.cancel { border-color: transparent; background: transparent; } }
</style>
