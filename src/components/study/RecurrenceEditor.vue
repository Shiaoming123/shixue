...
<template>
  <section class="recurrence-editor" aria-label="重复规则">
    ...
    <div v-if="preset === 'custom'" class="custom-fields">
      ...
      <div v-if="cadenceKind === 'weekly'" class="field"><span>星期</span><div class="weekday-buttons"><Button v-for="day in weekdaysOptions" :key="day.value" variant="secondary" size="sm" class="weekday-button" :class="{ selected: weekdays.includes(day.value) }" :aria-pressed="weekdays.includes(day.value)" @click="toggleWeekday(day.value)">{{ day.label }}</Button></div></div>
