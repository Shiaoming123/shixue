  ...
  <aside v-if="task" class="detail-drawer" :class="{ mobile, covering }" :role="covering ? undefined : 'complementary'" aria-label="任务详情">
    ...
    <div class="drawer-scroll">
      ...
      <dl class="facts">
        ...
        <div v-if="task.tags.length"><dt><Tag :size="18" />标签</dt><dd class="tag-list"><span v-for="tag in task.tags" :key="tag">#{{ tag }}</span></dd></div>
...
.detail-drawer.covering .drawer-header, .detail-drawer.covering .drawer-actions { background: var(--material-thin); -webkit-backdrop-filter: var(--glass-filter); backdrop-filter: var(--glass-filter); }
