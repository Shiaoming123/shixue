...
import type { ReminderRule, ReminderTarget, ReminderTrigger } from '../../domain/workspace/types'
...
const target = computed<ReminderTarget | null>(() => props.eventId ? { kind: 'event', eventId: props.eventId, originalStart: props.originalStart } : props.taskId ? { kind: 'task', taskId: props.taskId, occurrenceId: props.occurrenceId } : null)
const hasStart = computed(() => Boolean(props.eventId || props.startAt))
const rulesForTask = computed(() => props.rules.filter((rule) => JSON.stringify(rule.target) === JSON.stringify(target.value)))
const presetOptions = computed(() => [
  { value: 'at_start', label: '开始时', disabled: !hasStart.value },
  { value: '10', label: '提前 10 分钟', disabled: !hasStart.value && !props.dueAt },
  { value: '60', label: '提前 1 小时', disabled: !hasStart.value && !props.dueAt },
  { value: 'custom', label: '自定义时间' },
])
const anchorOptions = computed(() => [
  { value: 'start', label: '计划开始', disabled: !hasStart.value },
  ...(!props.eventId ? [{ value: 'due', label: '截止时间', disabled: !props.dueAt || Boolean(props.occurrenceId) }] : []),
])
const trigger = computed<ReminderTrigger | null>(() => {
  ...
  if (preset.value === 'at_start') return hasStart.value ? { kind: 'at_start' } : null
  if (!props.eventId && anchor.value === 'due') return props.dueAt && !props.occurrenceId ? { kind: 'before_due', minutes: Number(preset.value) } : null
  return hasStart.value ? { kind: 'before_start', minutes: Number(preset.value) } : null
})
...
function save() {
  if (props.busy || !trigger.value || !target.value) return
  emit('set', { type: 'reminder.set', ruleId: editingRule.value?.id ?? draftRuleId.value, target: target.value, trigger: trigger.value, enabled: true, ...(editingRule.value ? { expectedRevision: editingRule.value.revision } : {}) })
}
