...
<template>
  <div v-if="open && task" class="backdrop" @click.self="emit('close')">
    <form class="sheet" role="dialog" aria-modal="true" aria-labelledby="task-edit-title" @submit.prevent="save">
      ...
      <label><span><ListTree :size="15" />清单</span><select v-model="topicId"><option value="">收件箱</option><option v-for="topic in topics" :key="topic.id" :value="topic.id">{{ topic.title }}</option></select></label>
      <div class="field-grid">
        <label><span><CalendarDays :size="15" />日期</span><input v-model="plannedOn" type="date" /></label>
        <label><span>截止</span><input v-model="dueOn" type="date" /></label>
      </div>
      <label><span><Bell :size="15" />提醒</span><input v-model="reminderAt" type="datetime-local" /></label>
      <label><span><Flag :size="15" />优先级</span><select v-model="priority"><option value="none">无</option><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select></label>
...
label { display: block; margin-top: 14px; } label > span { display: flex; align-items: center; gap: 6px; margin-bottom: 7px; color: var(--muted); font-size: var(--text-xs); font-weight: 600; } input, textarea, select { width: 100%; min-height: 44px; padding: 10px 12px; border: 1px solid var(--hairline); border-radius: var(--radius-lg); outline: 0; background: var(--control-fill); color: var(--text); font: inherit; font-size: var(--text-base); } textarea { min-height: 72px; resize: vertical; } input:focus, textarea:focus, select:focus { border-color: var(--accent); background: var(--surface); box-shadow: var(--focus-ring); }
