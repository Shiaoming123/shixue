...
<template>
  <section class="today-view">
    <PageHeader
      class="today-header"
      :title="actionable.length ? `今天有 ${actionable.length} 件，先完成这一件` : '今天的学习已经有了答案'"
      :subtitle="dateLabel"
    >
...
.lead-task { overflow: hidden; border: 1px solid color-mix(in srgb, var(--accent) 34%, var(--hairline)); border-radius: var(--radius-xl); background: var(--surface); }
