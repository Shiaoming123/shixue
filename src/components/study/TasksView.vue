...
async function handleShortcut(event: KeyboardEvent) {
  ...
  if (event.key === '/') { event.preventDefault(); toolbarOpen.value = true; await nextTick(); searchInput.value?.focus(); return }
...
<template>
  <section class="tasks-view">
    <PageHeader :title="title" :subtitle="subtitle" variant="large">
      <template #actions><div class="header-actions">
        <Popover :open="smartViewMenuOpen" kind="menu" align="end" mobile-sheet mobile-sheet-label="切换智能清单" @update:open="smartViewMenuOpen = $event">
          <template #trigger="{ triggerProps }"><IconButton v-bind="triggerProps" aria-label="切换智能清单"><ListTree :size="18" /></IconButton></template>
          <div class="smart-view-menu" role="menu" aria-label="智能清单">
            <Button v-for="option in smartViewOptions" :key="option.value" variant="quiet" role="menuitemradio" :aria-checked="smartView === option.value" @click="emit('smartViewChange', option.value as StudyTaskSmartView); smartViewMenuOpen = false"><span>{{ option.label }}</span><Check v-if="smartView === option.value" :size="16" /></Button>
          </div>
        </Popover>
        <IconButton aria-label="筛选" :class="{ active: toolbarOpen || searchModel }" :aria-expanded="toolbarOpen || Boolean(searchModel)" @click="toolbarOpen = !toolbarOpen"><Filter :size="18" /></IconButton>
        <IconButton aria-label="批量操作" :class="{ active: batchMode }" @click="batchMode = !batchMode; clearSelection()"><ListFilter :size="18" /></IconButton>
      </div></template>
    </PageHeader>
    <QuickAddComposer
    ...
    />
    <div v-if="toolbarOpen || searchModel" class="filters">
      <label class="search-field"><Search :size="16" /><input ref="searchInput" v-model="searchModel" type="search" aria-label="筛选当前清单" placeholder="筛选当前清单" /><IconButton v-if="searchModel" aria-label="清除筛选文字" size="sm" @click="searchModel = ''"><X :size="14" /></IconButton><kbd v-else>/</kbd></label>
    ...
    <div v-if="batchMode" class="batch-bar"><span>{{ selectedIds.length }}</span><div><IconButton :disabled="!selectedIds.length" aria-label="批量完成" @click="finishBatch('complete')"><CheckCircle2 :size="17" /></IconButton><IconButton :disabled="!selectedIds.length" aria-label="移到今天" @click="finishBatch('today')"><CalendarDays :size="17" /></IconButton><IconButton class="danger" :disabled="!selectedIds.length" aria-label="批量删除" @click="confirmDeleteIds = [...selectedIds]"><Trash2 :size="17" /></IconButton></div></div>
    <div v-if="tasks.length || occurrences.length" class="task-sections">
      <section v-for="section in sections" :key="section.key" class="task-section">
        <div v-if="section.label" class="section-heading"><h2 :class="{ overdue: section.key === 'overdue' }">{{ section.label }} <span>{{ section.tasks.length + section.occurrences.length }}</span></h2><Button v-if="section.key === 'overdue' && section.tasks.length" size="sm" @click="emit('overdueMoveToToday', section.tasks.map(({ id }) => id))">全部移到今天</Button></div>
        ...
        <article v-for="task in section.tasks" :key="task.id" class="task-row" :class="{ selected: selectedId === task.id, completed: task.status === 'completed' }">
          ...
          <IconButton v-else class="complete-button" :aria-label="task.status === 'completed' ? `重新打开 ${task.title}` : `完成 ${task.title}`" @click="emit('toggleComplete', task.id)"><span :class="[task.priority, { checked: task.status === 'completed' }]"><Check :size="14" /></span></IconButton>
          <Button class="task-main" variant="quiet" @click="emit('open', task.id)"><span class="task-copy"><strong>{{ task.title }}</strong><span v-if="task.reasons.length" class="reason-tags"><small v-for="reason in task.reasons" :key="reason">{{ reasonLabel(reason) }}</small></span><small class="tabular-numbers"><span v-if="task.plannedLabel"><CalendarDays :size="13" />{{ task.plannedLabel }}</span><span v-if="task.reminderLabel"><Bell :size="13" />{{ task.reminderLabel }}</span><span><Inbox :size="13" />{{ task.topic }}</span></small></span><Flag v-if="task.priority !== 'none'" class="priority" :class="task.priority" :size="17" fill="currentColor" /><ChevronRight :size="17" class="chevron" /></Button>
          <div class="row-actions"><IconButton v-if="task.reasons.includes('overdue')" :aria-label="`延期 ${task.title}`" @click="emit('defer', task.id)"><CalendarDays :size="15" /></IconButton><IconButton v-if="task.reasons.includes('overdue')" class="danger" :aria-label="`取消 ${task.title}`" @click="emit('cancel', task.id)"><X :size="15" /></IconButton><IconButton :aria-label="`编辑 ${task.title}`" @click="emit('edit', task.id)"><Pencil :size="15" /></IconButton><IconButton class="danger" :aria-label="`删除 ${task.title}`" @click="confirmDeleteIds = [task.id]"><Trash2 :size="15" /></IconButton></div>
        </article>
        </TransitionGroup>
      </section>
    </div>
    ...
    <Dialog :open="Boolean(confirmDeleteIds.length)" :title="`删除 ${confirmDeleteIds.length} 项？`" role="alertdialog" size="sm" :show-close="false" @close="confirmDeleteIds = []">
      <template #footer>
        <Button class="confirm-button" autofocus @click="confirmDeleteIds = []">取消</Button>
        <Button class="confirm-button" variant="destructive" @click="confirmDeleteIds.length === 1 ? (emit('delete', confirmDeleteIds[0]), confirmDeleteIds = []) : finishBatch('delete')">删除</Button>
      </template>
    </Dialog>
  </section>
</template>
...
<style scoped>
.tasks-view { width: min(100%, 840px); min-height: 100%; margin: 0 auto; padding: 32px 32px 80px; }.header-actions { display: flex; gap: 2px; }.header-actions :deep(.btn), .search-field :deep(.btn), .row-actions :deep(.btn) { color: var(--muted); }.header-actions :deep(.btn:hover), .header-actions :deep(.btn.active), .row-actions :deep(.btn:hover) { background: var(--control-fill); color: var(--text); }.smart-view-menu { width: 190px; display: grid; gap: 2px; padding: 6px; }.smart-view-menu :deep(.btn) { min-height: 38px; justify-content: space-between; padding: 0 10px; color: var(--text); font-size: var(--text-base); text-align: left; }.smart-view-menu :deep(.btn__content) { width: 100%; justify-content: space-between; }.smart-view-menu :deep(.btn:hover), .smart-view-menu :deep(.btn[aria-checked="true"]) { background: var(--control-fill); }.smart-view-menu :deep(.btn svg) { color: var(--accent); }
.search-field { grid-column: 1 / -1; min-height: var(--control-hit); display: grid; grid-template-columns: 20px 1fr auto; align-items: center; gap: 6px; padding: 0 8px 0 11px; border: 1px solid transparent; border-radius: var(--radius-md); background: var(--control-fill); color: var(--muted); }.search-field:focus-within { border-color: var(--accent); box-shadow: var(--focus-ring); }.search-field input { min-width: 0; border: 0; outline: 0; background: transparent; color: var(--text); font-size: var(--text-sm); }.search-field kbd { padding: 2px 6px; border: 1px solid var(--hairline); border-radius: 6px; color: var(--muted); font: inherit; font-size: 10px; }.filters { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; margin-top: 9px; padding: 10px; border: 1px solid var(--hairline); border-radius: var(--radius-lg); background: var(--surface); }
.batch-bar { min-height: 46px; display: flex; align-items: center; justify-content: space-between; margin-top: 10px; padding: 5px 7px 5px 13px; border-radius: var(--radius-lg); background: color-mix(in srgb, var(--accent) 9%, var(--surface)); color: var(--accent); font-size: var(--text-sm); }.batch-bar > div { display: flex; gap: 3px; }.batch-bar :deep(.btn) { color: inherit; }.task-sections { margin-top: 24px; }.task-section { padding: 0; background: var(--surface); }.task-section + .task-section { margin-top: 28px; }.task-section h2 { margin: 0 0 7px; color: var(--accent); font-size: var(--text-sm); font-weight: 650; }.task-section h2.overdue { color: var(--danger); }.task-section h2 span { margin-left: 4px; color: var(--muted); font-weight: 500; }
...
.task-main { display: flex; width: 100%; padding: 0; }.task-main :deep(.btn__content) { width: 100%; display: grid; grid-template-columns: minmax(0, 1fr) 22px 20px; align-items: center; gap: 6px; padding: 8px 8px 8px 0; }
@media (max-width: 819px) { .tasks-view { width: 100%; padding: 18px var(--screen-inline) calc(106px + env(safe-area-inset-bottom, 0px)); }.task-sections { margin-top: 20px; }.task-row { min-height: 68px; }.task-main { min-height: 67px; }.task-main :deep(.btn__content) { grid-template-columns: minmax(0, 1fr) 22px; }.chevron { display: none; }.task-copy strong { white-space: normal; }.task-copy small { gap: 9px; }.task-copy small span:nth-child(2) { display: none; }.row-actions { display: none !important; }.filters { grid-template-columns: 1fr; }.complete-button span { width: 24px; height: 24px; } }
