...
const emit = defineEmits<{ 'select-date': [date: string]; open: [item: CalendarItem]; 'toggle-task': [value: { taskId: string; occurrenceId: string | null }] }>()
    ...
    <div class="month-grid__days">
      <section v-for="day in days" :key="day" class="month-grid__day" :class="{ 'month-grid__day--outside': !day.startsWith(month) }" :aria-label="dayLabel(day)">
        ...
        <div class="month-grid__facts">
          <div v-for="item in overflow(day).visible" :key="item.key" class="month-grid__entry"><CalendarTaskToggle :item="item" :title="titleFor(item)" @toggle="emit('toggle-task', $event)" /><button type="button" class="month-grid__fact" :data-priority="item.presentation?.priority" :class="`month-grid__fact--${item.kind}`" :aria-label="`打开 ${titleFor(item)}，${timeLabel(item)}`" @click="emit('open', item)">
            <span>{{ timeLabel(item) }}</span><CalendarTaskLabel :item="item" :title="titleFor(item)" />
          </button></div>
        </div>
            ...
            <div class="month-grid__disclosure">
              ...
              <ol>
                <li v-for="item in facts(day)" :key="item.key" :class="`month-grid__disclosure-row--${item.kind}`">
                  <CalendarTaskToggle :item="item" :title="titleFor(item)" @toggle="emit('toggle-task', $event)" />
                  <button type="button" class="month-grid__fact" :aria-label="`打开 ${titleFor(item)}`" @click="close('select'); emit('open', item)"><span>{{ timeLabel(item) }}</span><CalendarTaskLabel :item="item" :title="titleFor(item)" /></button>
                </li>
              </ol>
            </div>
          </template>
...
<style scoped>
.month-grid__entry { display: flex; align-items: center; min-width: 0; gap: 3px; }.month-grid__entry .month-grid__fact { flex: 1; min-width: 0; }
...
.month-grid__days { flex-shrink: 0; min-height: 792px; grid-template-rows: repeat(6, minmax(132px, max-content)); }
...
.month-grid__fact { width: 100%; border-top: 0; border-right: 0; border-bottom: 0; color: var(--text); font-family: inherit; text-align: left; cursor: pointer; }
.month-grid__fact[data-priority='high'] { border-left-color: var(--danger); }
.month-grid__fact[data-priority='medium'] { border-left-color: var(--warning); }
.month-grid__fact:focus-visible { outline: 0; box-shadow: var(--focus-ring); }
.month-grid__fact--all-day { border-left-color: var(--muted); background: var(--surface-alt); }
.month-grid__fact > span:first-child { flex: 0 0 auto; color: var(--muted); font-variant-numeric: tabular-nums; }
.month-grid__fact > .calendar-task-label { flex: 1; min-width: 0; }
...
.month-grid__disclosure .month-grid__fact > span:first-child { width: 72px; flex: 0 0 auto; color: var(--muted); font-size: var(--text-xs); font-variant-numeric: tabular-nums; }
.month-grid__disclosure li strong { color: var(--text); font-size: var(--text-sm); font-weight: var(--font-medium); }
@media (max-width: 819px) {
  .month-grid__days { min-height: 888px; grid-template-rows: repeat(6, minmax(148px, max-content)); }
