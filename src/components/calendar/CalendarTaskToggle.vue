...
<template>
  <button v-if="visible" type="button" class="calendar-task-toggle" :aria-label="`${completed ? '重新打开' : '完成'} ${title}`" :aria-pressed="completed" @pointerdown.stop @click.stop="toggle"><Check :size="14" aria-hidden="true" /></button>
</template>
