...
.time-picker input { min-height: var(--field-min-height); min-width: 0; padding: 0 var(--space-2); border: 1px solid transparent; border-radius: var(--radius-md); outline: 0; background: var(--field-fill); color: var(--text); font: inherit; font-variant-numeric: tabular-nums; text-align: center; }
.time-picker input:hover:not(:focus) { background: var(--field-hover-fill); }
.time-picker input:focus { border-color: var(--accent); background: var(--field-focus-fill); box-shadow: var(--field-focus-ring); }
