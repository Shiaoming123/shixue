...
import { checkForUpdates, IDLE_UPDATE_STATE, isUpdateBusy, type UpdateState } from './lib/updater'
...
const updaterAvailable = computed(() => hasRuntimeCapability(runtime, 'native-updater'))
...
function openTaskEditor(taskId: string) {
  if (page.value !== 'tasks' && page.value !== 'today' && page.value !== 'calendar') setDestination({ kind: 'inbox' })
...
async function runUpdateCheck() {
  if (!updaterAvailable.value || isUpdateBusy(updateState.value.phase)) return
  await checkForUpdates((state) => { updateState.value = state })
}
</script>
