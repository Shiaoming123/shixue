...
test('native runtime platform is exposed to the WebView before module routing', () => {
  ...
  assert.match(entry, /generate_handler!\[\s*greet,\s*runtime_platform,\s*report_native_smoke_phase,\s*read_android_persistence_smoke_request,\s*report_android_persistence_smoke_result,\s*read_legacy_reminder_deliveries,\s*set_quick_add_shortcut\s*\]/)
})
