...
import { assertHostedWindows, assertHostedWindowsLifecycle, validatePublicMsiRelease, validatePublicMsiTag, validatePublicMsiBytes, PUBLIC_MSI } from '../scripts/stage-public-windows-msi.mjs'
...
test('staging refuses local, self-hosted and non-Windows execution before any writes', () => {
  const env = { GITHUB_ACTIONS: 'true', RUNNER_ENVIRONMENT: 'github-hosted', RUNNER_OS: 'Windows', GITHUB_EVENT_NAME: 'workflow_dispatch', GITHUB_REF: 'refs/heads/main', GITHUB_REPOSITORY: 'Shiaoming123/shixue', GITHUB_WORKFLOW_REF: 'Shiaoming123/shixue/.github/workflows/windows-public-msi.yml@refs/heads/main', GITHUB_TOKEN: 'test-token' }
  ...
  const { GITHUB_TOKEN: _token, ...lifecycleEnv } = env
  assert.doesNotThrow(() => assertHostedWindowsLifecycle('win32', lifecycleEnv))
  assert.throws(() => assertHostedWindowsLifecycle('win32', { ...lifecycleEnv, RUNNER_ENVIRONMENT: 'self-hosted' }))
})

test('manual workflow stages, smokes and uploads only bounded MSI evidence', async () => {
  const workflow = await readFile(new URL('../.github/workflows/windows-public-msi.yml', import.meta.url), 'utf8')
  assert.match(workflow, /workflow_dispatch:/)
  assert.doesNotMatch(workflow, /\n\s+push:/)
  assert.match(workflow, /node scripts\/stage-public-windows-msi\.mjs/)
  assert.match(workflow, /npm run smoke:windows-msi/)
  assert.match(workflow, /src-tauri\/target\/windows-msi-smoke-report\.json/)
})
