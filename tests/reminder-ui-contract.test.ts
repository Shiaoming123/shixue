...
test('independent reminder edits emit revision-aware commands and removal preserves source history', () => {
  const rule = { id: 'rule-1', target: { kind: 'task', taskId: 'task-1', occurrenceId: null }, enabled: true, revision: 3, trigger: { kind: 'before_start', minutes: 10 } }
  ...
  state.save()
  assert.deepEqual(events[0], ['set', { type: 'reminder.set', ruleId: 'rule-1', target: { kind: 'task', taskId: 'task-1', occurrenceId: null }, trigger: { kind: 'before_start', minutes: 60 }, enabled: true, expectedRevision: 3 }])
  state.remove(rule)
...
test('reminder card forwards completion intent, fixed snooze duration and explicit ambiguous retry', () => {
 ...
 test('event reminder editor exposes start anchors and emits only the event target', () => {
  const { state, events, unmount } = setupComponent('ReminderEditor', { eventId: 'event:one', originalStart: null, rules: [], busy: false })
  state.save()
  assert.deepEqual(events[0][1].target, { kind: 'event', eventId: 'event:one', originalStart: null })
  assert.deepEqual(events[0][1].trigger, { kind: 'at_start' })
  assert.equal('taskId' in events[0][1], false)
  assert.equal(state.anchorOptions.value.some((option: any) => option.value === 'due'), false)
  state.preset.value = '10'; state.save()
  assert.deepEqual(events[1][1].trigger, { kind: 'before_start', minutes: 10 })
  unmount()
})
