...
export function assertHostedWindowsLifecycle(platform, env) {
  ...
  if (platform !== 'win32' || env.GITHUB_ACTIONS !== 'true' || env.RUNNER_ENVIRONMENT !== 'github-hosted'
    ...
    || env.GITHUB_WORKFLOW_REF !== 'Shiaoming123/shixue/.github/workflows/windows-public-msi.yml@refs/heads/main') {
    throw new Error('This MSI lifecycle requires the manual GitHub-hosted Windows workflow on main.')
  }
}

export function assertHostedWindows(platform, env) {
  assertHostedWindowsLifecycle(platform, env)
  if (!env.GITHUB_TOKEN) throw new Error('Public MSI staging requires the workflow GitHub token.')
}
...
async function main() {
  ...
  const targetRoot = resolve(process.env.SHIXUE_WINDOWS_CARGO_TARGET?.trim() || resolve(root, 'src-tauri', 'target'))
  const evidence = resolve(targetRoot, 'public-windows-msi')
