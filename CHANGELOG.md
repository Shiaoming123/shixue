...
- 新增通用任务工作区，提供收件箱、今天、最近 7 天、日历、清单、已完成与可选学习入口。
- 新增日、周、月、议程日历，以及普通任务、单次发生项、未来发生项和整个重复系列的排期操作。
- 新增日、周、月、年与“完成后”重复任务、离线中英文快速新增、多提醒及全局快速新增快捷键。
- 新增可逆标签管理与全局搜索，可组合搜索任务、检查项、标签、完成证据、主题、状态和日期。
- 新增基于真实发生项与完成证据的学习节律，以及可下钻的周计划结果、证据覆盖、学习分钟和复习覆盖回顾。
- 新增 WorkspaceState v3、Study v1/v2 迁移，以及统一的 capability protocol、事务写入、审计和撤销边界。

### Changed

- 桌面、中等宽度和紧凑布局统一采用同一套任务、日历、搜索、Sheet 与主题控件交互系统。
- Windows 多提醒与桌面生命周期候选统一使用 `0.3.0` 版本身份，避免与已发布的 `0.2.3` 制品混淆。
- 正式 tag 发布在打包前运行完整测试套件，并继续以草稿 Release 完成 Portable 上传和校验后再公开。

### Fixed

- 修正全局搜索在宽屏布局仍被旧对话框宽度限制的问题。
- 修复旧 v3 数据中待复习任务已被软删除时新版无法启动的问题；SQLite 与 IndexedDB 都会先备份原始快照，再恢复待复习任务并重新执行严格校验。
- Windows 安装 smoke 在启动前检查真实 Windows 产品数据目录；仅在目录原先不存在时运行并清理自己创建的数据，避免把无效的环境变量覆盖误报为隔离验证。
